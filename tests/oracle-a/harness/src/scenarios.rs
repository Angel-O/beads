//! Gap-targeting scenarios — each exercises a behavior the static corpus does
//! NOT pin (see `docs/CONFORMANCE_GAPS.md`). Run against real bd they become
//! golden ground truth; run against bts-rs they are the parity test.
//!
//! `all()` are the curated, maintained scenarios. `catalog()` loads the full
//! enumerated set (`docs/scenarios/enumerated.json`, 500+ scenarios) so the
//! whole bd CLI surface can be wired exhaustively as data.
...
pub fn all() -> Vec<Scenario> {
    vec![
        // #1 CRITICAL: close-blocker -> dependent becomes ready (the headline gc loop).
        Scenario::new(
            "graph_close_ready",
            "g",
            &[
        ...
        // CRITICAL: dependency retype. bd REJECTS; gc's beadstest wrongly expects success.
        Scenario::new(
            "dep_retype",
            "r",
            &[
        ...
        // gc-contract field coverage: confirm a BLOCKED issue's show omits
        // is_blocked/from/ref/needs (bd never emits them; bts-rs must match).
        Scenario::new(
            "blocked_show_fields",
            "bf",
            &[
        ...
        // gc-contract error class: bd sql is unsupported in embedded mode.
        Scenario::new(
        ...
        // === gc-usage parity scenarios (audit follow-up; ground truth = real bd) ===
        // gc's wisp-tier reads ALWAYS send `query ephemeral=true` (bdstore.go:1489).
        Scenario::new(
        ...
        // gc sends `query label=order-run:<name>` for order last-run lookups.
        Scenario::new(
            "query_label_field",
            "ql",
            &[
        ...
        // gc sends `query parent=<id>` on the wisp path.
        Scenario::new(
            "query_parent_field",
            "qp",
            &[
        ...
        // (reachable via update --status open on a deferred bead). gc trusts ready as
        // its claimable set and never re-checks defer.
        Scenario::new(
        ...
        Scenario::new(
            ...
            &[
                ...
                &["update", "umk-1", "--set-metadata", "gc-routed-to=x", "--json"],
            ],
        ),
        ...
        // "007" and trailing-dot "1." stay STRINGS; plain ints/bools coerce. bts-rs parsed
        // "007" as 7. (Bigint float-precision is avoided — Go/serde format integers-as-
        // floats differently; out of gc's realistic surface.)
        Scenario::new(
        ...
        // REGRESSION GUARD: query --limit 0 = UNLIMITED (gc's primary wisps read). A
        // regression to unconditional truncate would silently drop matches yet still pass
        // the rest of the suite. bts-rs is currently correct; pin it.
        Scenario::new(
