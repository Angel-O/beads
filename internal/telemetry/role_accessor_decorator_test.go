...
// answer for: every storage.DoltStorage method that hands out a role interface
// from the public facade.
//
// The DERIVATION is duplicated from the sibling test in internal/storage rather
// than shared, because that package cannot import this one and its test helpers
// cannot be exported back. What is not duplicated any more is the census
// itself: both sides now read the same interface, so a twenty-fifth role cannot
...
func TestInstrumentedStorageDeclaresEveryRoleAccessor(t *testing.T) {
	...
	if len(roleAccessorNames) == 0 {
		t.Fatal("no storage.DoltStorage method hands out a facade role; the census is empty and this test would pass vacuously")
	}
