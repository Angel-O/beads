...
func (s *EmbeddedDoltStore) BatchApplier() (issueops.BatchApplier, error) {
	return NewBatchApplier(s)
}

// NewBatchApplier returns a guarded batch applier backed by store.
func NewBatchApplier(store *EmbeddedDoltStore) (issueops.BatchApplier, error) {
	if store == nil {
...
type batchApplier struct{ store *EmbeddedDoltStore }
