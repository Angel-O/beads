...
// seventh, EventCommentWrite, is engine-only: the hosted exporter advances its
// source cursor across a comment row without minting a wire event, so a comment
// never reaches an external consumer as an event of its own (its effect is
// already visible in the next issue snapshot). That reconciliation is the frozen
// wire contract, not an implementation detail to be re-litigated per consumer.
//
