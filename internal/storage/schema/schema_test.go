...
// TestIgnoredMigration0022RepairsLeaseGrantedNodeAfterOrdinalCollisionThroughDoltCLI
// reproduces a clone whose ignored cursor passed v16 before granted_node was
// assigned that ordinal. The next migration must repair the missing column.
func TestIgnoredMigration0022RepairsLeaseGrantedNodeAfterOrdinalCollisionThroughDoltCLI(t *testing.T) {
	testutil.RequireDoltBinary(t)

	dir := filepath.Join(t.TempDir(), "lease-granted-node-ordinal-collision")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatalf("create ordinal-collision dir: %v", err)
	}
	runDoltCommand(t, dir, "init", "--name", "test", "--email", "test@example.com")

	// Match the historical clone-local state directly. The ignored cursor is
	// already beyond the colliding v16, but its five-column leases table never
	// received granted_node.
	runDoltSQL(t, dir, `
CREATE TABLE leases (
...
CREATE TABLE ignored_schema_migrations (
...
`)
	requireDoltCount(t, dir, `SELECT COUNT(*) AS c FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'leases' AND COLUMN_NAME = 'granted_node'`, "0")

	migrationSQL, err := ignoredSource.files.ReadFile("migrations/ignored/0022_ensure_lease_granted_node.up.sql")
	if err != nil {
		t.Fatalf("read ignored 0022 up migration: %v", err)
	}
	cliSQL := cliCompatibleMigrationSQL("0022_ensure_lease_granted_node.up.sql", string(migrationSQL))
	runDoltSQL(t, dir, cliSQL)
	requireDoltCount(t, dir, `SELECT COUNT(*) AS c FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'leases' AND COLUMN_NAME = 'granted_node'`, "1")
}
