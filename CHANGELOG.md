...
- **Plane tracker adapter.** `bd plane sync` / `bd plane status` synchronize issues bidirectionally with self-hosted [Plane](https://github.com/makeplane/plane) Community Edition (v1.3.x API) — the seventh tracker adapter. Work items are keyed by Plane's native `external_id`/`external_source` fields (`external_id` = bead ID), so creation is idempotent: an interrupted sync recovers through Plane's 409-with-existing-UUID instead of duplicating. Status maps through Plane state groups (custom project states work unconfigured); beads issue types and blocked status round-trip via `beads:type:*`/`beads:blocked` labels since Plane CE has neither concept; descriptions convert Markdown↔HTML with sanitization, and GitHub-Flavored Markdown (tables, strikethrough, task lists) round-trips intact. Ephemeral beads (wisps) stay local unless `--include-ephemeral` is passed, `--state open|closed` filters both pull and push, and a 429 that survives retries aborts the push cleanly, leaving the remainder for the next sync. Ships with an in-process fake Plane server (`internal/plane/planetest`) validated by a conformance suite that can also run against a live deployment (env-gated `TestLivePlaneConformance`). See [docs/PLANE_CONFIG.md](docs/PLANE_CONFIG.md).
